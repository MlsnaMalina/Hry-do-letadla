      ...
      const t = setTimeout(() => {
        ...
        occ.forEach(i => {
          ...
          for (let dx = -1; dx <= 1; dx++) for (let dy = -1; dy <= 1; dy++) {
            ...
            if (nx >= 0 && nx < N && ny >= 0 && ny < N && board[ny * N + nx] == null) cand.add(ny * N + nx)
          }
        })
        let arr = [...cand]; if (!arr.length) arr = board.map((v, i) => v == null ? i : -1).filter(i => i >= 0)
        const idx = arr[Math.floor(Math.random() * arr.length)]
        if (idx != null) {
          const b = board.slice(); b[idx] = 1
          const w = check(b, idx, 1)
          setBoard(b)
  ...
  return (
    <GameShell players={players} active={active} winner={winner}
      ...
      <ResultsModal open={winner != null} sub={mode === 'ai' ? 'Hra proti AI' : 'Pass & play'}
        title={winner === 0 || mode !== 'ai' ? `${players[winner ?? 0].name} vyhrává!` : 'AI vyhrává'}
